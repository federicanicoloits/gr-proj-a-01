...
for (let i=0; i<10; i++){
  arrayOfProducts[i] = {
    name: padWord(nameExtraction()), id: generateID(), dateCreation: printDate(formatDate()), dateFine: 3, 
    calStatus(){
      let start = this.dateCreation;
      let end = this.dateFine;
      let difference = end - start;
  ...
  statusUpdate(i, 2);
  console.log(arrayOfProducts[i].name, arrayOfProducts[i].id, arrayOfProducts[i].dateCreation, arrayOfProducts[i].dateFine, arrayOfProducts[i].status);
}
...
function statusUpdate(num, dateActual) {
  ...
  if(dateActual<apt && dateActual>arrayOfProducts[num].dateCreation){
    arrayOfProducts[num].status = 'valid';
  } else if(dateActual>=2 && arrayOfProducts[num].dateFine) {
